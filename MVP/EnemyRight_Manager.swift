...
class Enemy_Right_Manager {
    ...
    func addEnemy() -> Void {
        ...
        if random == 1 {
            ...
            ItemNode.position = CGPoint(x: self.scene.size.width * 0.59, y: self.scene.size.height + self.scene.size.height / 10)
            ...
            ItemNode.anchorPoint = CGPoint(x: 0.5, y: 0.5)
            ItemNode.position = CGPoint(x: self.scene.size.width * 0.75, y: self.scene.size.height + self.scene.size.height / 10)
            ItemNode.size = CGSize(width: self.scene.size.width / 7, height: self.scene.size.height / 8)
            ItemNode.zRotation = CGFloat(M_PI / 1)
